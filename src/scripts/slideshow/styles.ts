...
.slideshow-sorter__top { display:grid; grid-template-columns:minmax(0,1fr) auto; gap:8px; align-items:start; }
.slideshow-sorter__title { font-weight:600; min-width:0; white-space:normal; overflow-wrap:anywhere; line-height:1.35; }
.slideshow-sorter__badges { display:flex; gap:5px; flex-wrap:wrap; justify-content:flex-end; color:var(--text-muted); font-size:var(--font-ui-smaller); }
.slideshow-sorter__badge { display:inline-flex; align-items:center; gap:3px; }
.slideshow-sorter__badge svg { width:14px; height:14px; }
.slideshow-sorter__content { display:grid; grid-template-columns:auto minmax(120px,1fr) auto; gap:8px; align-items:center; }
.slideshow-sorter__drag { cursor:grab; display:flex; align-items:center; }
.slideshow-sorter__drag:active { cursor:grabbing; }
